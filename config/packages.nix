[

  #a
  "ack"
  "ansible"
  "apacheHttpd"
  "apt"
  "attr"
  "audit"
  "autoconf"
  "automake"
  #b
  "bash"
  "bat"
  "bc"
  "bencodetools"
  "bintools"
  "binutils"
  "bison"
  "blender"
  "boost"
  "busybox"
  "byacc"
  #c
  "cabal-install"
  "calcmysky"
  "cfitsio"
  "check"
  "chromium"
  "claws-mail"
  "clippy"
  "cmake"
  #"composer"
  "coreutils"
  "coreutils-full"
  #"cpanminus"
  "cppcheck"
  "cppunit"
  "cracklib"
  "criu"
  "cunit"
  "cups"
  "curl"
  #d
  "darktable"
  "dejagnu"
  "delve"
  "diffutils"
  "djvulibre"
  "dnf4"
  "dnf5"
  "dnsmasq"
  "docker"
  "docker-compose"
  "dosbox"
  "dotnet-sdk"
  "dovecot"
  "doxygen"
  #e
  "e2fsprogs"
  "ed"
  "elfutils"
  "emacs"
  "evince"
  "exabgp"
  "exiv2"
  "expat"
  #f
  "fastfetch"
  "fd"
  "ffmpeg"
  "ffmpeg-full"
  "file"
  "findutils"
  "firefox"
  "firefox-esr"
  "fish"
  "flatpak"
  "flatpak-builder"
  "flex"
  "flite"
  "freecad"
  "freeciv"
  "fzf"
  #g
  "gawk"
  "gcc"
  "gdb"
  "geeqie"
  "getopt"
  "gettext"
  "ghc"
  "gimp"
  "git"
  "gjs"
  "gnu-config"
  "gnugrep"
  "gnum4"
  "gnupg"
  "gnutar"
  "go"
  "godot"
  "golangci-lint"
  "gopls"
  "gpm"
  "gradle"
  "grafana"
  "graphviz"
  "grub2"
  #"gst-plugins-bad"
  #"gst-plugins-base"
  #"gst-plugins-good"
  #"gst-plugins-rs"
  #"gst-plugins-ugly"
  #"gstreamer"
  "gtk4"
  "gzip"
  #h
  "happy"
  "haproxy"
  "hello"
  "helm"
  "help2man"
  "home-assistant"
  "hscolour"
  "hugo"
  "hwdata"
  "hydra"
  "hydra-check"
  "hydra-cli"
  "hyprland"
  #i
  "i2p"
  "i2pd"
  # "i3"  build gets stuck on Writing logfile
  "iana-etc"
  "imagemagick"
  "indilib"
  "inkscape"
  "ispc"
  #j
  "jellyfin"
  "jemalloc"
  "jq"
  #k
  "kdePackages.kate"
  "kdePackages.kuserfeedback"
  "kdePackages.kwrite"
  "kdePackages.okular"
  "kdePackages.qtmultimedia"
  "krb5"
  "krita"
  "kubectl"
  "kubernetes"
  #l
  "lame"
  "leiningen"
  "less"
  "lib2geom"
  "lib3ds"
  "lib3mf"
  # liba
  "libaacs"
  "libaal"
  "libabigail"
  "libabw"
  "libacars"
  "libaccounts-glib"
  "libacr38u"
  "libad9361"
  "libadwaita"
  "libaec"
  "libagar"
  "libagar_test"
  "libaio"
  "libajantv2"
  "libamplsolver"
  "libamqpcpp"
  "libansilove"
  "libantlr3c"
  "libao"
  "libaom"
  "libaosd"
  "libapparmor"
  "libappimage"
  "libappindicator"
  "libappindicator-gtk2"
  "libappindicator-gtk3"
  # "libapplewm"  darwin only
  "libapreq2"
  "libarchive"
  "libarchive-qt"
  "libargon2"
  "libargs"
  "libaribcaption"
  "libart_lgpl"
  "libasn1c"
  "libasr"
  "libass"
  "libassuan"
  # "libast"  removed due to lack of maintenance upstream
  "libastyle"
  "libasyncns"
  "libatasmart"
  "libation"
  "libatomic_ops"
  "libaudclient"
  "libaudec"
  "libavc1394"
  "libavif"
  "libax25"
  "libayatana-appindicator"
  # "libayatana-appindicator-gtk3"  replaced with "libayatana-appindicator"
  "libayatana-common"
  "libayatana-indicator"
  "libayatana-indicator-gtk3"
  # libb
  "libb2"
  "libb64"
  "libbacktrace"
  "libbap"
  "libbase58"
  # "libbaseencode"  replaced with "libcotp"
  # "libbass_fx"  unfree
  # "libbassmidi"  unfree
  # "libbassmix"  unfree
  # "libbass"  unfree
  "libbde"
  "libbdplus"
  # "libbencodetools"  replaced with "bencodetools"
  "libbfd"
  "libbfd_2_38"
  "libbfio"
  "libbgcode"
  "libbladeRF"
  "libblake3"
  "libblockdev"
  "libblocksruntime"
  "libbluray"
  "libbluray-full"
  "libbpf"
  "libbpf_0"
  # "libbpf_1"  replaced with "libbpf"
  "libbpkg"
  "libbraiding"
  "libbs2b"
  "libbsd"
  # "libbson"  replaced with "mongoc"
  "libbtbb"
  "libburn"
  "libbutl"
  "libbytesize"
  "libc"
  # libc
  "libcaca"
  "libcacard"
  "libcamera"
  "libcamera-qcam"
  "libcanberra"
  "libcanberra-gtk2"
  "libcanberra-gtk3"
  "libcanberra_kde"
  "libcangjie"
  "libcap"
  "libcap-ng"
  "libcap_ng"
  "libcaption"
  "libcardiacarrest"
  "libcava"
  "libcbor"
  "libccd"
  "libcdada"
  "libcdaudio"
  "libcddb"
  "libcdio"
  "libcdio-paranoia"
  "libcdr"
  "libcec"
  "libcec_platform"
  # "libcef"  removed
  "libcello"
  "libceph"
  "libcerf"
  "libcgroup"
  "libchamplain_libsoup3"
  # "libchamplain"  replaced with "libchamplain_libsoup3"
  "libchardet"
  "libchdr"
  "libchewing"
  "libchipcard"
  "libcIconv"
  "libcifpp"
  "libcint"
  "libcircle"
  "libck"
  "libclang"
  "libclc"
  "libcli"
  "libclipboard"
  "libcloudproviders"
  "libclthreads"
  "libclxclient"
  "libcmatrix"
  "libcmis"
  "libcoap"
  "libcollectdclient"
  "libcomps"
  "libconfig"
  "libconfuse"
  "libcork"
  "libcorrect"
  "libcosmicAppHook"
  "libcotp"
  "libcouchbase"
  "libcpr"
  "libcpr_1_10_5"
  "libcpucycles"
  "libcpuid"
  "libcrafter"
  "libcredis"
  "libcrossguid"
  "libcryptui"
  "libcs50"
  "libcsa"
  "libcsptr"
  "libcss"
  "libctb"
  "libctemplate"
  "libctl"
  "libcue"
  "libcue-lang"
  "libcupsfilters"
  "libcutl"
  "libcxx"
  "libcxxrt"
  "libcxxStdenv"
  "libcyaml"
  # libd
  "libdaemon"
  "libdaq"
  "libdatachannel"
  "libdatovka"
  "libdatrie"
  "libdazzle"
  "libdbi"
  "libdbi-drivers"
  "libdbiDrivers"
  "libdbi-drivers-base"
  "libdbiDriversBase"
  "libdbusmenu"
  "libdbusmenu-gtk2"
  "libdbusmenu-gtk3"
  "libdbusmenu-qt5"
  "libdc1394"
  "libde265"
  "libdecor"
  "libdeflate"
  "libdeltachat"
  "libdevil"
  "libdevil-nox"
  "libdex"
  "libdicom"
  "libdict"
  "libdigidocpp"
  "libdiscid"
  "libdislocator"
  "libdisplay-info"
  "libdisplay-info_0_2"
  "libdivecomputer"
  "libdivsufsort"
  "libdjinterop"
  "libdlcr"
  "libdmapsharing"
  "libdmtx"
  "libdmx"
  "libdnet"
  "libdnf"
  "libdng"
  "libdom"
  "libdovi"
  "libdrm"
  "libdsk"
  "libdv"
  "libdvbcsa"
  "libdvbpsi"
  "libdvdcss"
  "libdvdnav"
  "libdvdnav_4_2_1"
  "libdvdread"
  "libdvdread_4_9_9"
  "libdwarf"
  "libdwarf-lite"
  "libdynd"
  # libe
  "libe57format"
  "libeatmydata"
  "libeb"
  "libebml"
  "libe-book"
  "libebur128"
  "libeconf"
  "libedgetpu"
  "libedit"
  "libeduvpn-common"
  "libee"
  "libei"
  "libelf"
  "libelfin"
  "libemf2svg"
  "libepc"
  "libepoxy"
  "libepubgen"
  "liberal-crime-squad"
  "liberastika"
  "liberasurecode"
  "liberation-circuit"
  "liberation-sans-narrow"
  "liberation_ttf"
  "liberation_ttf_v1"
  "liberation_ttf_v2"
  "liberfa"
  "liberio"
  "libertine"
  "libertine-g"
  "libertinus"
  "libesmtp"
  "libestr"
  "libetebase"
  "libetonyek"
  "libetpan"
  "libeufin"
  "libev"
  "libevdev"
  "libevdevc"
  "libevdevplus"
  "libevent"
  "libewf"
  "libewf-legacy"
  "libexecinfo"
  "libexif"
  "libexosip"
  "libexsid"
  "libextractor"
  "libexttextcat"
  # libf
  "libf2c"
  "libfabric"
  "libfakekey"
  "libfaketime"
  "libfakexinerama"
  "libfakeXinerama"
  "libfann"
  "libfastjson"
  "libff"
  "libffado"
  "libffcall"
  "libffi"
  "libffi_3_3"
  "libffiReal"
  "libfido2"
  "libfilezilla"
  "libfishsound"
  "libfive"
  "libfixposix"
  "libflowmanager"
  "libfm"
  "libfm-extra"
  "libfmvoice"
  "libfolia"
  "libfontenc"
  "libfoundry"
  "libfprint"
  "libfprint-2-tod1-broadcom"
  "libfprint-2-tod1-broadcom-cv3plus"
  "libfprint-2-tod1-elan"
  "libfprint-2-tod1-goodix"
  "libfprint-2-tod1-goodix-550a"
  "libfprint-2-tod1-vfs0090"
  "libfprint-focaltech-2808-a658"
  "libfprint-tod"
  "libfpx"
  "libfreeaptx"
  "libfreefare"
  "libfreehand"
  "libfrida-core"
  "libfs"
  "libfsm"
  "libftdi"
  "libftdi1"
  "libfx2"
  "libfyaml"
  # libg
  "libgadu"
  "libgaminggear"
  "libgbinder"
  "libgbm"
  "libgcc"
  "libgccjit"
  "libgcrypt"
  "libgda"
  "libgda5"
  "libgda6"
  "libgdamm"
  "libgdata"
  "libgdiplus"
  "libgdsii"
  "libgdstk"
  "libgedit-amtk"
  "libgedit-gfls"
  "libgedit-gtksourceview"
  "libgedit-tepl"
  "libgee"
  "libgen-cli"
  "libgeneral"
  "libgeotiff"
  "libgepub"
  "libgestures"
  "libgff"
  "libgflow"
  "libghostty-vt"
  "libgig"
  "libgit2"
  "libgit2-glib"
  "libglibutil"
  "libglut"
  "libglvnd"
  "libglycin"
  "libglycin-gtk4"
  "libgme"
  "libgmpris"
  "libgnome-games-support"
  "libgnome-games-support_2_0"
  "libgnomekbd"
  "libgnome-keyring"
  "libgnome-keyring3"
  "libgnt"
  "libgnunetchat"
  "libgnurl"
  "libgourou"
  "libgovirt"
  "libgpg-error"
  "libgpg-error-gen-posix-lock-obj"
  "libgphoto2"
  "libgpiod"
  "libgpiod_1"
  "libgpod"
  "libgrapheme"
  "libgravatar"
  "libgringotts"
  "libgsf"
  "libgssglue"
  "libgtkflow3"
  "libgtkflow4"
  "libgtop"
  "libgudev"
  "libguestfs"
  "libguestfs-appliance"
  "libguestfs-with-appliance"
  "libgumath"
  "libguytools"
  "libgweather"
  "libgxps"
  # libh
  "libhandy"
  "libhandy_0"
  "libhangul"
  "libharu"
  "libhdhomerun"
  "libheif"
  "libheimdal"
  "libhighscore"
  "libhomfly"
  "libhsts"
  "libhttpserver"
  "libhubbub"
  "libhugetlbfs"
  "libhv"
  "libhwy"
  "libhx"
  "libhydrogen"
  "libical"
  "libidn"
  "libjpeg"
  "liblinear"
  "libmpeg2"
  "libndp"
  "libnl"
  "libpcap"
  "libpng"
  "libproxy"
  "libreoffice"
  "libssh2"
  "libtool"
  "libtorrent-raksasa"
  "libultrahdr"
  "libuv"
  "libxcrypt"
  "libxxf86vm"
  "linux"
  "llvm"
  "lua"
  "lynx"
  #m
  "mailcap"
  "mariadb"
  "maven"
  "maxima"
  "mc"
  "mesa"
  "meson"
  "mongoc"
  # "mongodb" unfree
  "mono"
  "mplayer"
  "mutt"
  "mysql84"
  #n
  "nano"
  "ncurses"
  # "neofetch"  replaced with fastfetch
  "neovim"
  "networkmanager"
  "nghttp2"
  "nginx"
  "nix"
  "nix-info"
  "nlohmann_json"
  "nmap"
  "nodejs"
  "nodejs-slim"
  #"nodePackages.esbuild"
  #"nodePackages.eslint"
  #"nodePackages.prettier"
  #"nodePackages.typescript"
  "nspr"
  #"numpy"
  #o
  "octave"
  "oniguruma"
  "openblas"
  "opencolorio"
  "openjdk"
  "openresolv"
  "openssh"
  "openssl"
  "openttd"
  "openvpn"
  #p
  "p7zip"
  # "packer" unfree
  "pacman"
  "pandoc"
  "patch"
  "patchelf"
  "pcre2"
  "perl"
  "php"
  "pidgin"
  "pihole"
  "pipes"
  "pkg-config"
  "podman"
  "postfix"
  "postgresql"
  "privoxy"
  "prometheus"
  "protobuf"
  "python3"
  "python313Packages.cython"
  "python313Packages.ipython"
  "python313Packages.markupsafe"
  "python313Packages.msgspec"
  "python313Packages.sphinx"
  "python314Packages.sphinx"
  "python3Minimal"
  # "python"  python2 was removed
  #q
  "qemu"
  #"qt5Full" qt5Full has been removed
  #"qt5.qtmultimedia"
  #r
  "rapidjson"
  "rdesktop"
  "redis"
  "ripgrep"
  "rsync"
  "rtorrent"
  #"rubyPackages.nokogiri"
  "rust-analyzer"
  "rustc"
  #s
  "samba"
  "sane-backends"
  "sbcl"
  "scribus"
  "scummvm"
  #"silver-searcher" has been removed as it has seen no development since 2020 and is stuck on the obsolete pcre library
  "smartmontools"
  "sqlite"
  "squid"
  "stack"
  "stdenv"
  "stellarium"
  "sudo"
  "systemd"
  #t
  #"terraform" unfree
  "thunderbird"
  "tmux"
  "tor"
  "toybox"
  "transmission_4"
  # u
  "unbound"
  "unzip"
  #v
  "valgrind"
  "vim"
  "virtualbox"
  "vlc"
  #w
  "wayland"
  "wesnoth"
  "wget"
  "which"
  "wine"
  "wire-desktop"
  "wireshark"
  #x
  "xorg-server"
  "xterm"
  "xwayland"
  "xz"
  #y
  "yarn"
  "yggdrasil"
  "yt-dlp"
  #z
  "zeromq"
  "zlib"
  "zsh"
  "zstd"
]
